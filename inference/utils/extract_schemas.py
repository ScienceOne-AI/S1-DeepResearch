            ...
            if "python" in block.lower():
                ...
                if tool_name in ["search", "google_scholar"]:
                    ...
                    if isinstance(search_query, list):
                        ...
                        calls.append({"name": "web_search", "arguments": {"query": search_query}})
                    else:
                    ...
                    if isinstance(visit_url, list):
                        for url in visit_url:
                            calls.append({"name": "visit_url", "arguments": {"start_url": url, "task_prompt": visit_goal}})
                    elif isinstance(visit_url, str):
                        calls.append({"name": "visit_url", "arguments": {"start_url": visit_url, "task_prompt": visit_goal}})
                    else:
